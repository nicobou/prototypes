...
static gboolean
serializer_event_probe (GstPad * pad, GstEvent * event, GstElement * serializer)
{
    gboolean ret=TRUE;
  ...
  switch (GST_EVENT_TYPE (event)) {
	...
	//todo remove probe... with id given by gst_pad_add_event_probe
	gst_pad_remove_event_probe(pad,probeIdForFlushing);
	...
	gst_pad_set_blocked_async (sinkPadPeer, FALSE,echo_pad_unblocked,NULL);
...
switch_to_new_serializer (GstPad * pad, gboolean blocked, gpointer user_data )
{
    ...
    probeIdForFlushing=gst_pad_add_event_probe (srcPad,G_CALLBACK (serializer_event_probe),serializer);

    g_print("probe added \n");
    //send EOS to serializer 
    if(gst_element_send_event(serializer,gst_event_new_eos()))
	g_print("EOS sent to serializer \n");
    else 
	g_print("issue sending EOS to serializer\n");
