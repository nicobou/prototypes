...
static shmdata_any_writer_t *writer;
                              ...
                              GdkEventExpose *event)
{
  ...
  if (framebuffer_allocated == FALSE) {
    ...
    if (!shmdata_any_writer_set_path (writer, "/tmp/vnc_test"))
      {
	fprintf (stderr, "**** The file exists. exiting...\n");
	shmdata_any_writer_close (writer);
      }  else { 
      shmdata_any_writer_set_debug (writer, SHMDATA_ENABLE_DEBUG);
      g_print ("rshift %u, gshift %u, bshift %u \n",
	       cl->format.redShift,
	       cl->format.greenShift,
	       cl->format.blueShift);
      gchar *caps = g_strdup_printf("video/x-raw-rgb, width=(int)%u, "
				    "framerate=(fraction)30/1, "
				    "height=(int)%u, bpp=(int)%u, "
				    "depth=(int)%u, endianness=(int)4321, "
				    "red_mask=(int)%d, green_mask=(int)%d, "
				    "blue_mask=(int)%d",
				    cl->width,
				    cl->height,
				    ...
				    /* cl->format.greenShift,  */
				    /* cl->format.blueShift  */
				    65280, 16711680, -16777216
				    );
      shmdata_any_writer_set_data_type (writer, caps);
      g_free(caps);
